...
function echeck(str) {
    ...
    if (str.indexOf(at) == -1) {
        ...
        //Ext.MessageBox.alert('Status', 'Invalid E-mail ID', 'changeLocation');
        return false;
    }
...
(function(window, document, undefined) {
    ...
    var log = function(msg) {
        ...
        loader = function() {
            ...
            while (i--) {
                div.appendChild(top.document.createElement("span"));
            }
            return div;
        },
        ...
        element = function(name, text) {
            var el = top.document.createElement(name);
            el.appendChild(top.document.createTextNode(text));
            return el;
        },
        ...
        currentPage,
        modalBody = parent.document.createElement("div");
    ...
    window.Feedback = function(options) {
        ...
        // default properties
        options.label = options.label || "Report Problem"; //|| "Send Feedback";
        options.header = options.header || "Report Problem";
        options.url = options.url || "SendFeedbackS.jsp";
        ...
        var button,
            modal,
            currentPage,
            glass = top.document.createElement("div"),
            returnMethods = {
                ...
                open: function() {
                    ...
                    var a = element("a", "x"),
                        modalHeader = top.document.createElement("div"),
                        // modal container
                        modalFooter = top.document.createElement("div");

                    modal = top.document.createElement("div");
                ...
                send: function(adapter) {
                    ...
                    adapter.send(data, function(success) {
                        ...
                        if (success === true) {
                            modalBody.appendChild(top.document.createTextNode(options.messageSuccess));
                        } else {
                            modalBody.appendChild(top.document.createTextNode(options.messageError));
                        }
        ...
        for (; i < len; i++) {
            ...
            switch (item.type) {
                case "textarea":
                    ...
                    this.dom.appendChild((item.element = top.document.createElement("textarea")));
                    break;
                case "text":
                    this.dom.appendChild(element("label", item.label + ":" + ((item.required === true) ? " *" : "")));
                    ...
                    input.disabled = "true";
                    this.dom.appendChild((item.element = input));
                    break;
                case "br":
                    this.dom.appendChild((item.element = top.document.createElement("br")));
                    break;
            }
        }
    ...
    window.Feedback.Form.prototype.end = function() {
        ...
        for (; i < len; i++) {
            ...
            if (item.required === true && item.element.value.length === 0) {
                ...
                return false;
            } else if (item.element.name == "emailid" && !echeck(item.element.value)) {
                item.element.className = "feedback-error";
                return false;
            } else {
        ...
        for (; i < len; i++) {
            ...
            try {
                if (item.element.value.length > 0) {
                    ...
                    //dom.appendChild( top.document.createTextNode( item.element.value.length ) );
                    dom.appendChild(top.document.createElement("hr"));
                }
            } catch (e) {
    ...
    window.Feedback.Screenshot.prototype.start = function(modal, modalHeader, modalFooter, nextButton) {
        ...
        if (this.h2cDone) {
            ...
            this.mouseClickEvent = function(e) {
                ...
                if (action === false) {
                    if (blackoutBox.getAttribute(dataExclude) === "false") {
                        var blackout = top.document.createElement("div");
                    ...
                    if (highlightBox.getAttribute(dataExclude) === "false") {
                        ...
                        $this.highlightBox = highlightBox = top.document.createElement('canvas');
            ...
            this.highlightClose = element("div", "CLOSE");
            this.blackoutBox = top.document.createElement('div');
            this.highlightBox = top.document.createElement("canvas");
            this.highlightContainer = top.document.createElement('div');
            var timer,
            ...
            for (var i = 0; i < 2; i++) {
                ...
                this.dom.appendChild(top.document.createTextNode(" "));
    ...
    window.Feedback.Screenshot.prototype.render = function() {
        ...
        if (window.html2canvas === undefined && script === undefined) {
            ...
            script = top.document.createElement("script");
    ...
    window.Feedback.Screenshot.prototype.data = function() {
        ...
        if (this.h2cCanvas !== undefined) {
            ...
            if (items.length > 0) {
                ...
                // copy canvas
                canvasCopy = top.document.createElement("canvas");
